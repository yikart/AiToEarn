...
const Trending: React.FC = () => {
  ...
  const [selectedViralCategory, setSelectedViralCategory] =
    useState<string>('');
  const [viralTitleData, setViralTitleData] = useState<{ category: string; titles: ApiViralTitle[] }[]>(
    [],
  );
  ...
  const [singleCategoryData, setSingleCategoryData] = useState<ApiViralTitle[]>(
    [],
  );
  ...
  const fetchPlatformRanking = async (platformId: string) => {
    ...
    try {
      const response = await platformApi.getPlatformRanking(platformId);
  ...
  const fetchRankingCategories = async (rankingId: string) => {
    ...
    try {
      const response = await platformApi.getRankingLabel(rankingId);
  ...
  const fetchRankingDates = async (rankingId: string) => {
    ...
    try {
      const response = await platformApi.getRankingDates(rankingId);
      const rankingDatesData = response?.data || [];
  ...
  const fetchRankingContents = async (
    ...
    try {
      ...
      if (response?.data) {
        const data = response.data as RankingContentsResponse;
        setRankingContents(data.items);
  ...
  const fetchViralTitleTimeTypes = async () => {
    try {
      ...
      const timeTypeData = response?.data || [];
  ...
  const fetchViralTitleContents = async (platformId: string, timeType: string) => {
    ...
    try {
      ...
      if (response?.data) {
        const data = response.data as { category: string; titles: ApiViralTitle[] }[];
        setViralTitleData(data);
        const allTitles = data.flatMap(item => item.titles);
        setViralTitleContents(allTitles);
      } else {
        setViralTitleData([]);
        setViralTitleContents([]);
      }
    } catch (error) {
