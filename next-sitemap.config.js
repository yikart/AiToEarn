...
module.exports = {
  ...
  transform: async (_, locPath) => {
    if (HAS_DYNAMIC_SEGMENT.test(locPath)) return null;
    if (BLOCKLIST_REGEX.test(locPath)) return null;
    if (locPath.includes("[lng]")) return null;
    return {
      loc: locPath,
      changefreq: "monthly",
      priority: 0.7,
  ...
  additionalPaths: async (config) => {
    ...
    const pageFiles = allPageFiles
      ...
      .filter((f) => {
        ...
        if (BLOCKLIST_REGEX.test(unix)) return false;
