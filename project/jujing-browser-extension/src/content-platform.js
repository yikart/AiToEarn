(() => {
  ...
  const platform = location.hostname.includes('xiaohongshu')
    ? 'xhs'
    : location.hostname.includes('douyin')
      ? 'douyin'
      : 'unknown'
  ...
  function extractWorkIdFromUrl(url) {
    return url.match(/(?:explore|video|item|search_result)\/([^/?#]+)/)?.[1] || ''
  }
  ...
  function isNoiseText(text) {
    ...
    return /^(相关搜索|全部|图文|视频|用户|筛选|综合|发现|直播|发布|通知|登录|创作中心|业务合作)$/.test(value)
      || /登录后查看搜索结果|手机号登录|获取验证码|扫码登录|用户协议|隐私政策/.test(value)
  ...
  function scanVisibleWorks(keyword = '', limit = 10) {
    ...
    for (const card of cards) {
      const link = card.matches('a') ? card : card.querySelector('a[href*="/explore/"], a[href*="/video/"], a[href*="/discovery/item/"]')
      ...
      const workId = href.match(/(?:explore|video|item)\/([^/?#]+)/)?.[1] || `visible-work-${Date.now()}-${works.length}`
      ...
      works.push({
        author,
        authorId: card.querySelector('a[href*="/user"]')?.getAttribute('href') || '',
    ...
    if (!works.length) {
      ...
      return comments.map((comment, index) => ({
        ...
        workId: location.href.match(/(?:explore|video|item)\/([^/?#]+)/)?.[1] || `visible-work-${index}`,
      }))
    }
  ...
  async function discoverByKeyword(payload = {}) {
    ...
    if (/登录后查看搜索结果|手机号登录|获取验证码|扫码登录|登录后查看更多/.test(pageText)) {
      return {
        items: [],
        keyword,
        message: '小红书搜索页要求登录，请先在打开的小红书页面完成登录后重试',
        success: false,
      }
    }
  ...
  async function openCommentComposer() {
    const commentButton = findButtonByText(['评论', '写评论', '说点什么'])
    if (commentButton)
      commentButton.click()
  ...
  async function submitComment(content, replyToCommentId) {
    ...
    const submitButton = findButtonByText(['发送', '发布', '评论'])
    if (!submitButton) {
      return {
  ...
  async function sendDirectMessage(content) {
    ...
    const messageButton = findButtonByText(['私信', '发消息', '聊天'])
    if (messageButton) {
      messageButton.click()
      await sleep(1000)
    }
    ...
    setNativeValue(input, content.trim())
    await sleep(300)
    const sendButton = findButtonByText(['发送'])
    if (!sendButton) {
      return {
