...
export default function FollowingTab() {
  ...
  const { following, fetchFollowing } = useFollowing()
  const observerRef = useRef<IntersectionObserver | null>(null)

  useEffect(() => {
    if (following.users.length === 0 && !following.loading) {
      fetchFollowing(true)
    }
  }, [])
  ...
  const lastUserRef = useCallback(
    (node: HTMLDivElement | null) => {
      ...
      observerRef.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && following.hasMore) {
    ...
    [following.loading, following.hasMore, fetchFollowing],
  )
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      {following.users.map((user, i) => {
        const isLast = i === following.users.length - 1
        return (
