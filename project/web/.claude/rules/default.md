...
TypeScript 类型检查：使用 `npx tsc --noEmit`，不要使用 `npm run build` 做类型检查。
不要主动跑 `npm run build` 命令！！！
...
**❌ 禁止使用 `npm run build` 或 `pnpm run build` 进行类型检查**

原因：
- `build` 命令会进行完整的生产构建，包括打包、优化、生成静态文件等
- 构建过程非常耗时（通常需要几分钟）
- 会生成大量构建产物文件，污染工作目录
- 只为检查类型错误而进行完整构建是极大的资源浪费
...
2. **IDE 实时检查**：
   - Cursor/VSCode 会自动进行类型检查
   - `read_lints` 工具会检查语法错误
   - 修改代码时实时反馈
...
```bash
# 开发时快速验证
pnpm run type-check    # 检查类型错误（推荐）
pnpm run lint         # 检查代码质量

# 只在发布前进行完整构建
pnpm run build        # 生产构建（仅在需要时）
```
...
- **不要给 `t()` 加类型断言**：当前项目的翻译函数 `t()` 已支持任意 key 的运行时解析，不需要也不要使用 `as any`、`as string` 等类型断言来绕过类型检查。正确写法示例：
  ```tsx
  // ✅ 正确
  t('promptGallery.title')

  // ❌ 不要这样
  t('promptGallery.title' as any)
  ```

- **新增翻译 key 的要求**：当新增页面或组件（例如 `PromptGallery`）时，若使用了新的翻译 key（如 `promptGallery.title` / `promptGallery.subtitle`），请同时在各语言的翻译文件中添加对应条目，文件位置通常位于 `/src/app/i18n/<lng>/<namespace>.json`。例如在默认语言文件中添加：
  ```json
  {
    "promptGallery": {
      "title": "提示词示例",
      "subtitle": "点击示例，快速生成或定制你的内容"
    }
  }
  ```

  这样可以确保在缺少翻译时有合理的回退文案，同时便于后续维护和翻译管理。
