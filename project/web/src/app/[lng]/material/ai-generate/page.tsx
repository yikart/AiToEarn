...
export default function AIGeneratePage() {
  ...
  const pollImageEditTaskStatus = async (logId: string) => {
    ...
    const poll = async () => {
      const done = await checkStatus(); if (!done)
  ...
  const fetchImageModels = async () => {
    try {
      const res: any = await getImageGenerationModels(); if (res.data) {
        setImageModels(res.data); if (res.data.length)
  ...
  const fetchImageEditModels = async () => {
    try {
      const res: any = await getImageEditModels(); if (res.data) {
        setImageEditModels(res.data); if (res.data.length)
                ...
                <div className={styles.section}>
                  <div className={styles.twoColumn}>
                    <div className={styles.leftPanel}>
                        ...
                        <button className={styles.modelSelectBtn} onClick={() => setShowModelDropdown(s => !s)}>
                          ...
                          <div className={styles.modelMain}>
                            ...
                            <div className={styles.modelDesc} style={{ marginTop: 4 }}>
                              {(() => { const m: any = imageModels.find((x: any) => x.name === model); return m?.description || '' })()}
                            </div>
                          </div>
                      ...
                      {model && (() => {
                        const selected = imageModels.find((m: any) => m.name === model); const credit = selected?.pricing ? Number.parseFloat(selected.pricing) : 0; return credit > 0
                          ? (
                ...
                <div className={styles.section}>
                  <div className={styles.twoColumn}>
                    <div className={styles.leftPanel}>
                      ...
                      {imageEditModel && (() => {
                        const selected = imageEditModels.find((m: any) => m.name === imageEditModel); const credit = selected?.pricing ? Number.parseFloat(selected.pricing) : 0; return credit > 0
                          ? (
                              <div className={styles.creditCostInfo}>
                                <span style={{ color: '#1890ff', fontSize: '14px' }}>
                                  💰
                ...
                <div className={styles.leftPanel}>
                  ...
                  {(() => {
                    if (videoMode !== activeVideoTab)
                      setVideoMode(activeVideoTab); return null
                  })()}
                  ...
                  <div className={styles.modelSelect}>
                    <button className={styles.modelSelectBtn} onClick={() => setShowVideoModelDropdown(s => !s)}>
                      ...
                      <div className={styles.modelMain}>
                        ...
                        <div className={styles.modelDesc} style={{ marginTop: 4 }}>
                          {(() => { const m: any = (filteredVideoModels as any[]).find((x: any) => x.name === videoModel); return m?.description || '' })()}
                        </div>
                      </div>
                      <span className={styles.modelCaret}>▾</span>
                    </button>
