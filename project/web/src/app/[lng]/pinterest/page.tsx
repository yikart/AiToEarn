...
// 自定义颜色选择器组件
function CustomColorPicker({ value, onChange }: { value?: string, onChange?: (value: string) => void }) {
...
export default function PinterestPage() {
  ...
  const loadPins = async (page = 1, size = 10) => {
    ...
    try {
      setLoading(true)
      const response = await getPinterestPinListApi({ page, size }, selectedAccount.id)
      if (response?.code === 0) {
        ...
        {selectedBoard && (
          <div>
            ...
            <Text type="secondary">
              {t('board.createTime')}
              :
              {new Date(selectedBoard.created_at).toLocaleDateString()}
            </Text>
          </div>
        )}
      </Modal>
          ...
          <div>
            ...
            {selectedPin.board_owner && (
              <div style={{ marginBottom: 16 }}>
                ...
                <Text>
                  @
                  {selectedPin.board_owner.username}
                </Text>
              </div>
            )}
            ...
            <Text type="secondary">
              {t('board.createTime')}
              :
              {new Date(selectedPin.created_at).toLocaleDateString('zh-CN')}
            </Text>
          </div>
        )}
      </Modal>
