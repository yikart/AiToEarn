...
import { Button, Form, Input, Select, Modal, message, Space, InputNumber, List, Card, Spin, Avatar } from "antd";
...
export default function CgMaterialPageCore() {
  ...
  const getMaterialTypeLabel = (type: string | undefined) => {
    if (type === PubType.ImageText) return t('materialType.imageText' as any);
    if (type === PubType.VIDEO) return t('materialType.video' as any);
    return type || '';
  };

  const getMaterialStatusLabel = (status: number | undefined) => {
    if (status === 0) return t('materialStatus.generating' as any);
    if (status === 1) return t('materialStatus.completed' as any);
    return '';
  };
  ...
  async function handleEditGroup() {
    if (!editGroupName) return message.warning(t('editGroup.enterName' as any));
    setEditLoading(true);
    try {
    ...
    <div className={styles.materialContainer}>
                    ...
                    <div style={{ display: 'flex', gap: 12, width: '100%' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 12, color: '#999', display: 'flex', gap: 8 }}>
                          ...
                          <span>• {item.mediaType === 'image' ? t('import.image' as any) : item.mediaType === 'video' ? t('import.video' as any) : t('import.article' as any)}</span>
                          <span>• {item.viewCount} {t('import.views' as any)}</span>
                          <span>• {item.likeCount} {t('import.likes' as any)}</span>
                          <span>• {item.commentCount} {t('import.comments' as any)}</span>
                        </div>
                      </div>
                    </div>
                  </List.Item>
        ...
        <Spin spinning={previewLoading}>
          {!previewData ? (
            ...
            <Card title={previewData.title} bordered>
              <div style={{marginBottom:8}}><b>{t('detail.descLabel' as any)}</b>{previewData.desc}</div>
              <div style={{marginBottom:8}}><b>{t('detail.typeLabel' as any)}</b>{getMaterialTypeLabel(previewData.type)}</div>
              <div style={{marginBottom:8}}><b>{t('detail.coverLabel' as any)}</b>
              ...
              <div style={{marginBottom:8}}><b>{t('fields.location' as any)}</b>{Array.isArray(previewData.location) ? previewData.location.join(', ') : ''}</div>
              <div><b>{t('detail.materialsLabel' as any)}</b>
      ...
      >
        {detailData && (
          <div>
            <div style={{ marginBottom: 16 }}>
              <b>{t('detail.titleLabel' as any)}</b>{detailData.title}
            </div>
            <div style={{ marginBottom: 16 }}>
              <b>{t('detail.descLabel' as any)}</b>{detailData.desc}
            </div>
            <div style={{ marginBottom: 16 }}>
