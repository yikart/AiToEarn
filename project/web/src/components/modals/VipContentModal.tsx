...
const VipContentModal = memo(({ open, onClose }: VipContentModalProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'year' | 'month' | 'once'>('month')
  ...
  const isCurrentPlan = useMemo(() => {
    ...
    return {
      ...
      trial: vipInfo.status === 'trialing', // 试用中
    }
  ...
  const handleActivate = async (planType: 'year' | 'month' | 'once') => {
    ...
    try {
      ...
      const response: any = await createPaymentOrderApi({
        returnTo: userStore.lang === 'zh-CN' ? 'https://aitoearn.ai/zh-CN/profile' : 'https://aitoearn.ai/en/profile',
        mode: paymentMethod,
        payment: paymentType,
        metadata: {
  ...
  return (
    ...
    >
      <div className={vipStyles.wrapper}>
            ...
            >
              <div className={vipStyles.titleBlock}>
                <h2 className={vipStyles.title}>
                  ...
                  {' '}
                  <span className={vipStyles.highlight}>{translate('modal.highlight')}</span>
                </h2>
                <div className={vipStyles.links}>
                  <span>{translate('modal.choosePlan')}</span>
                  <span
                    className={vipStyles.linkButton}
        ...
        {isVip && userStore.userInfo?.vipInfo && (
          <h5 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#0f172a', textAlign: 'center', marginTop: 16, marginBottom: 16 }}>
            {' '}
        ...
        <div className={vipStyles.switchRow}>
          ...
          {/* <div onClick={() => handleTabClick('month')} className={`${vipStyles.switchBtn} ${activeTab === 'month' ? vipStyles.active : ''}`}>
            {translate('modal.tabs.monthly')}
            <Tag style={{ marginLeft: 6, border: '1px solid rgba(184,221,255,.08)' }}>{translate('modal.savings.monthly')}</Tag>
          </div> */}
          {
          // (() => {
          //   const vipInfo = userStore.userInfo?.vipInfo;
        ...
        <div className={vipStyles.grid}>
          ...
          {activeTab === 'year' && (
            <>
              ...
              <div className={vipStyles.freeCard}>
                ...
                <div className={vipStyles.freePrice}>
                  <span>{translate('modal.free.price')}</span>
                  <span className={vipStyles.unit}>{translate('modal.free.period')}</span>
                </div>
              ...
              <div className={`${vipStyles.planCard} ${vipStyles.premium}`}>
                <div className={vipStyles.planHead}>
                  {translate('modal.plans.yearly.title')}
                  {' '}
                  <Tag color="#5b7cff">{translate('modal.plans.yearly.bestValue')}</Tag>
                </div>
                <div className={vipStyles.planPriceLine}>
                  <span className={vipStyles.currency}>$</span>
                  <span className={vipStyles.bigNum}>{translate('modal.plans.yearly.price')}</span>
                  <span className={vipStyles.unit}>{translate('modal.plans.yearly.period')}</span>
                </div>
                ...
                <div className={vipStyles.benefitBox}>
                  <span className={vipStyles.dot} />
                  {' '}
                  {translate('modal.plans.yearly.features.dailyPoints')}
                </div>
            ...
            <>
              ...
              <div className={vipStyles.freeCard}>
                ...
                <div className={vipStyles.freePrice}>
                  {' '}
                  $
                  <span style={{ fontSize: 44, fontWeight: 800, color: '#333' }}>{translate('modal.free.price')}</span>
                  <span className={vipStyles.unit}>{translate('modal.free.period')}</span>
                </div>
                <Button disabled className={vipStyles.freeBtn}>
                  {isVip ? translate('modal.free.freePlan') : translate('modal.free.currentPlan')}
              ...
              <div className={`${vipStyles.planCard} ${vipStyles.premium}`}>
                <div className={vipStyles.planHead}>
                  {translate('modal.plans.monthly.title')}
                  {' '}
                </div>
                <div className={vipStyles.planPriceLine}>
                  <span className={vipStyles.currency}>$</span>
                  <span className={vipStyles.bigNum}>{translate('modal.plans.monthly.price')}</span>
                  <span className={vipStyles.unit}>{translate('modal.plans.monthly.period')}</span>
                </div>
                {/* <div className={vipStyles.planDesc}>
                ...
                <Button
                  ...
                  loading={loading}
                  disabled={isCurrentPlan.month || isCurrentPlan.trial}
                >
                  {isCurrentPlan.month
                    ? translate('currentPlan')
                    : (
                        userStore.userInfo?.vipInfo ? translate('modal.plans.goumai') : 'Start 3-day free trial'
                      )}
                </Button>
                <div className={vipStyles.benefitBox}>
                  <span className={vipStyles.dot} />
          ...
          {activeTab === 'once' && (
            <>
              ...
              <div className={`${vipStyles.planCard} ${vipStyles.premium}`}>
                <div className={vipStyles.planHead}>
                  {translate('modal.plans.once.title')}
                  {' '}
                </div>
                <div className={vipStyles.planPriceLine}>
                  <span className={vipStyles.currency}>$</span>
                  <span className={vipStyles.bigNum}>{translate('modal.plans.once.price')}</span>
                  <span className={vipStyles.unit}>{translate('modal.plans.once.period')}</span>
                </div>
                <Button
                  className={vipStyles.primaryBtn}
