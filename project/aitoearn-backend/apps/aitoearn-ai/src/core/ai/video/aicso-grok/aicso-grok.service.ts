...
export interface AicsoGrokVideoCreateRequest {
  userId: string
  userType: UserType
  model: string
  prompt: string
...
export interface AicsoGrokVideoCallbackDto {
  id: string
...
@Injectable()
export class AicsoGrokVideoService {
  private readonly logger = new Logger(AicsoGrokVideoService.name)
  ...
  async createVideo(request: AicsoGrokVideoCreateRequest) {
    const { userId, userType, model, prompt, images, aspectRatio, size } = request
    ...
    const aiLog = await this.aiLogRepo.create({
      ...
      taskId: result.id,
      model,
      channel: AiLogChannel.AicsoGrok,
      startedAt,
      type: AiLogType.Video,
      points: pricing,
      request: { model, prompt, images, aspectRatio, size },
  ...
  async callback(queryResult: AicsoQueryResponse): Promise<AicsoGrokVideoCallbackDto> {
    const aiLog = await this.aiLogRepo.getByTaskId(queryResult.id)
    if (!aiLog || aiLog.channel !== AiLogChannel.AicsoGrok) {
      throw new AppException(ResponseCode.InvalidAiTaskId)
    }

    if (aiLog.status !== AiLogStatus.Generating) {
      return aiLog.response as AicsoGrokVideoCallbackDto
    }

    this.logger.debug({ queryResult }, 'AicsoGrok callback')

    const videoUrl = queryResult.video_url
    ...
    if (queryResult.status === AicsoTaskStatus.Completed && videoUrl) {
      ...
      const callbackData: AicsoGrokVideoCallbackDto = {
        id: queryResult.id,
    ...
    return {
      id: queryResult.id,
      status: queryResult.status === AicsoTaskStatus.Processing
        ? AicsoTaskStatus.Processing
        : AicsoTaskStatus.Pending,
    }
  }

  getTaskResult(result: AicsoGrokVideoCallbackDto) {
    const status = {
  ...
  extractInput(request: Record<string, unknown>) {
    return {
      ...
      size: request['size'] as string | undefined,
    }
  }

  async getTask(userId: string, userType: UserType, logId: string): Promise<AicsoGrokVideoCallbackDto> {
    const aiLog = await this.aiLogRepo.getByIdAndUserId(logId, userId, userType)

    if (aiLog == null || !aiLog.taskId || aiLog.type !== AiLogType.Video || aiLog.channel !== AiLogChannel.AicsoGrok) {
      throw new AppException(ResponseCode.InvalidAiTaskId)
    }
