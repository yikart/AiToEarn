    ...
    try {
      ...
      if (imgUrlList && imgUrlList.length > 0) {
        ...
        for (const imgUrl of imgUrlList) {
          ...
          if (!container) {
            res.message = 'Create image container failed'
            return res
          }
        ...
        if (!container) {
          res.message = 'Create video container failed'
          return res
        }
        await this.postMediaContainerService.createMetaPostMedia({
          accountId: publishTask.accountId,
      ...
      res.message = 'Publish task is being processed'
      return res
    }
    catch (error) {
  ...
  async publish(task: PublishTask): Promise<DoPubRes> {
    ...
    if (!isMediaCompleted) {
      ...
      return {
        ...
        message: 'Media files are still processing',
      }
    }
    ...
    if (containers.length > 1) {
      ...
      if (!postContainer || !postContainer.id) {
        ...
        return {
          status: PublishStatus.FAILED,
          message: 'Create media container failed',
          noRetry: true,
        }
      }
    ...
    if (!publishRes || !publishRes.id) {
      ...
      return {
        status: PublishStatus.FAILED,
        message: 'Publish media container failed',
        noRetry: true,
      }
    }
    this.logger.log(
    ...
    return {
      ...
      message: 'All media files have been processed',
    }
  }
}
