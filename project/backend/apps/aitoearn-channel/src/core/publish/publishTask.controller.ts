...
export class PublishTaskController {
  ...
  async nowPubTask(@Body() data: NowPubTaskDto) {
    ...
    await this.publishTaskService.pushPubTask(info)
  ...
  async getPublishTaskList(@Body() data: PublishRecordListFilterDto) {
    const res = await this.publishTaskService.getPublishTasks(data)
    return res
  }

  @Post('channel/status/queued/tasks')
  async getQueuedPublishTaskList(@Body() data: PublishRecordListFilterDto) {
    const res = await this.publishTaskService.getQueuedPublishTasks(data)
    return res
  }
