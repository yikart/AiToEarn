...
import { IMMEDIATE_PUBLISH_TOLERANCE_SECONDS, PUbLISHING_SCHEDULED_TASK_CRON_EXPRESSION } from '../../core/publish/constant'
...
export class PublishTaskService implements OnModuleDestroy {
  ...
  async pushPubTask(publishTask: PublishTask) {
    ...
    if (!pubService)
      throw new AppException(ResponseCode.PublishServiceNotFound, { accountType: publishTask.accountType })

    const res = await pubService.pushPubTask(publishTask)
    if (!res)
      throw new AppException(ResponseCode.PublishTaskFailed, { accountType: publishTask.accountType })
    return res
  }
  ...
  async createPublishingTask(data: CreatePublishDto) {
    try {
      ...
      this.logger.error(e)
      return new AppException(ResponseCode.ChannelAccountInfoFailed)
    }
  }
