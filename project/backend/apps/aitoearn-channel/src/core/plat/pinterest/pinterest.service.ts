...
export class PinterestService {
  ...
  async getBoardList(accountId: string) {
    const tokenInfo = await this.getUserStat(accountId)
    const accessToken = tokenInfo.access_token as string
    this.logger.log(JSON.stringify({ hasToken: !!accessToken }))
  ...
  async createPin(body: CreatePinBody) {
    ...
    const data = await this.pinterestApiService.createPin(body, accessToken)
    return { code: 0, data }
  }
  ...
  async delPinById(id: string, accountId: string) {
    ...
    const tokenInfo = await this.getUserStat(accountId)
    const accessToken = tokenInfo.access_token as string
    const data = await this.pinterestApiService.deletePin(id, accessToken)
    return { code: 0, data }
  }
