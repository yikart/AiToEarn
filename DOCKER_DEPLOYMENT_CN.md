...
所有环境变量直接在 `docker-compose.yml` 文件中配置。

编辑 `docker-compose.yml`，**至少修改以下必填项**：
...
STATISTICS_DB_PASSWORD: your-secure-password  # 默认与 MongoDB 密码一致
...
# 5. aitoearn-web 服务 — 非 localhost 部署时修改
NEXT_PUBLIC_API_URL: https://your-domain.com/api
```

> **重要**: 修改密码时，必须同步更新所有引用该密码的服务。例如 MongoDB 密码出现在 `mongodb`、`aitoearn-ai`、`aitoearn-server` 三个服务中。
>
> 可以使用 `openssl rand -hex 32` 生成随机字符串。

### 3. 配置应用配置文件

`config/` 目录包含应用配置文件，以只读卷挂载到容器中：

| 文件 | 挂载到 | 说明 |
|------|--------|------|
| `config/aitoearn-ai.config.js` | aitoearn-ai:/app/config.js | AI 服务配置（AI 模型定义、存储配置） |
| `config/aitoearn-server.config.js` | aitoearn-server:/app/config.js | 后端配置（OAuth 回调、邮件、存储配置） |
| `config/gcs-placeholder.json` | aitoearn-ai:/app/config/gcs-placeholder.json | Google Cloud 凭证占位文件 |

这些配置文件从 `process.env`（由 docker-compose.yml 设置）读取环境变量，同时包含一些硬编码配置（存储端点、AI 模型列表等）。

**如需修改 RustFS 凭证或切换到外部 S3/OSS**，需同时更新两个 config.js 文件中的 `assets` 配置部分。

**如需使用真实 Gemini 服务**，将 `config/gcs-placeholder.json` 替换为实际的 Google Cloud 服务账号凭证文件。
...
| `NEXT_PUBLIC_API_URL` | aitoearn-web | 前端 API 地址（浏览器端访问） | `http://localhost:8080/api` |
...
默认与主 MongoDB 使用同一实例，生产环境可分离。

| 变量 | 所属服务 | 说明 | 默认值 |
|------|----------|------|--------|
| `STATISTICS_DB_HOST` | aitoearn-server | 统计数据库主机 | `mongodb` |
| `STATISTICS_DB_PORT` | aitoearn-server | 统计数据库端口 | `27017` |
| `STATISTICS_DB_USERNAME` | aitoearn-server | 统计数据库用户名 | `admin` |
| `STATISTICS_DB_PASSWORD` | aitoearn-server | 统计数据库密码 | `password` |
...
3. `config/aitoearn-ai.config.js` 和 `config/aitoearn-server.config.js` 中 `assets` 配置的 `accessKeyId` 和 `secretAccessKey`

**生产环境**如使用 AWS S3 或阿里云 OSS，更新两个 config.js 文件中的 `assets` 配置：

```javascript
// config.js 中的 assets 配置示例（AWS S3）
assets: {
  provider: 's3',
...
AI 模型的具体定义（可用模型列表、定价等）在 `config/aitoearn-ai.config.js` 中配置。
...
| `ALI_GREEN_ACCESS_KEY_ID/SECRET` | aitoearn-server | 阿里云内容安全 | https://yundun.console.aliyun.com |
| `NEW_API_URL` / `NEW_API_TOKEN` | aitoearn-server | 内部 API 中转服务 | 自行部署 new-api |

## 配置文件参考

以下是 `config/` 目录中配置文件的完整说明。这些文件以只读卷挂载到容器中，修改后需重启对应服务。
...
与 aitoearn-ai.config.js 中的 `assets` 配置相同，两处需保持一致。
...
| 配置项 | 说明 | 来源 |
|--------|------|------|
| `newApi.baseUrl` | New API 服务地址 | 环境变量 `NEW_API_URL` |
| `newApi.token` | New API 访问 token | 环境变量 `NEW_API_TOKEN` |

---

### config/gcs-placeholder.json

Google Cloud 服务账号凭证占位文件。Gemini 服务需要此文件来访问 Google Cloud API。

默认包含占位值，如需使用真实 Gemini 服务，替换为从 Google Cloud Console 下载的服务账号 JSON 密钥文件。
...
### Q: config/ 目录下的配置文件有什么作用？

`config/` 目录包含两个应用配置文件和一个凭证占位文件：

- **aitoearn-ai.config.js**: AI 服务的详细配置，包含 AI 模型定义（可用模型列表、定价）、存储配置、Gemini 设置等
- **aitoearn-server.config.js**: 后端服务配置，包含 OAuth 回调地址模式、邮件传输设置、存储配置等
- **gcs-placeholder.json**: Google Cloud 服务账号凭证占位文件，使用 Gemini 时需替换为真实凭证

这些文件以只读卷挂载到容器中，修改后需重启对应服务：`docker compose restart aitoearn-ai aitoearn-server`

### Q: 如何修改或添加 AI 模型？

编辑 `config/aitoearn-ai.config.js` 中的 `ai.models` 部分，可以修改可用模型列表、定价和模型参数。修改后重启 AI 服务：
...
1. 确认 `docker-compose.yml` 中 `aitoearn-web` 服务的 `NEXT_PUBLIC_API_URL` 配置正确
2. 检查 Nginx 是否正常运行：`docker compose logs nginx`
3. 检查后端服务是否健康：`docker compose ps`
...
4. AI 模型的配置在 `config/aitoearn-ai.config.js` 中，确认模型定义与你的 API 服务匹配
