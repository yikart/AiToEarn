...
如需在 aitoearn-server 启动时自动初始化管理员 token，可通过 docker-compose 的 `command` 读取 token 文件并设置为 `AUTO_LOGIN_TOKEN` 环境变量：
...
> 注意：之前的 `AUTO_LOGIN_TOKEN_PATH` 环境变量已被移除，改为在启动命令中直接读取文件内容。
