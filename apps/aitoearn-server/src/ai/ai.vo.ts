...
import { z } from 'zod'
import { messageContentComplexSchema } from './dto'
import { TaskStatus } from './libs/dashscope'
...
const dashscopeVideoGenerationResponseSchema = z.object({
  task_id: z.string().describe('任务ID'),
  task_status: z.enum(TaskStatus).optional().describe('任务状态'),
})
...
const dashscopeTaskStatusResponseSchema = z.object({
  ...
  output: z
    .object({
      task_id: z.string().describe('任务ID'),
      task_status: z.enum(TaskStatus).describe('任务状态'),
