...
export class YoutubeService {
  ...
  async createAccountAndSetAccessToken(
    ...
    try {
      ...
      // 如果不存在，则创建
      let accountToken = await this.OAuth2CrendentialModel.findOne({
      ...
      if (accountToken) {
        ...
        // 创建新的令牌记录
        accountToken = await this.OAuth2CrendentialModel.create({
          platform: AccountType.YOUTUBE,
          accountId: googleId,
  ...
  async getVideoCategoriesList(
    ...
    if (!(await this.ensureValidAccessToken(accountId))) {
      ...
      // return { code: ExceptionCode.Failed, message: 'refresh token error.', data: [] }
      return new AppException(ExceptionCode.Failed, 'get access token error.')
    }
    try {
