...
export class RedisService {
  ...
  async setKey<T>(key: string, value: T, seconds?: number): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error('Redis set key error:', error)
  ...
  async resetOrderNumber(h = '') {
    try {
      ...
      this.logger.error('Error resetting order sequence:', error)
    }
  }
  ...
  async renameKey(oldKey: string, newKey: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error renaming key from ${oldKey} to ${newKey}:`, error)
      return false
    }
  }
}
