...
import {
  ...
  WxGzhArticleNews,
  WxGzhArticleNewsPic,
} from '@/libs/wxGzh/common'
import { WxGzhApiService } from '@/libs/wxGzh/wxGzhApi.service'
...
export class WxGzhService {
  ...
  async uploadTempMedia(
    ...
    const blobFile = await this.fileToolsService.fileUrlToBlob(this.fileToolsService.filePathToUrl(url))
  ...
  async uploadImg(accountId: string, imgUrl: string) {
    const accessToken = await this.getAccessToken(accountId)
    const blobFile = await this.fileToolsService.fileUrlToBlob(this.fileToolsService.filePathToUrl(imgUrl))
  ...
  async addMaterial(
    ...
    const blobFile = await this.fileToolsService.fileUrlToBlob(this.fileToolsService.filePathToUrl(fileUrl))
    const accessToken = await this.getAccessToken(accountId)
