...
export class ThreadsPublishService extends PublishBase implements MetaPostPublisher {
  ...
  async publish(task: PublishTask): Promise<DoPubRes> {
    const containers = await this.postMediaContainerService.getContainers(task.id);
    ...
    const unProcessedContainers = containers.filter(media => media.status !== PostMediaStatus.FINISHED);
