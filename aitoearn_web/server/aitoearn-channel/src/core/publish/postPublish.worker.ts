...
export class PostPublishWorker extends WorkerHost implements OnModuleDestroy {
  ...
  async process(
    ...
    try {
      ...
      throw new Error(`[task-${taskId}] Job ${job.id} failed: ${error.message}`, { cause: error });
    }
  }
  ...
  onStalled(job: Job<{ taskId: string }>) {
    this.logger.error(`[task-${job.data.taskId}] Job stalled: ${job.data.taskId}`);
  }
