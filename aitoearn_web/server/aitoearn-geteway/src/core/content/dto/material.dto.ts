...
export class CreateMaterialDto {
  ...
  @IsArray({ message: '经纬度必须是数组' })
  @IsNumber(
    { allowNaN: false },
    { each: true, message: '经纬度必须是数字数组' },
  )
  @ArrayMinSize(2, { message: '经纬度数组长度必须为2' })
  @ArrayMaxSize(2, { message: '经纬度数组长度必须为2' })
  @IsOptional()
  @Expose()
...
export class CreateMaterialTaskDto implements NewMaterialTask {
  ...
  // 经纬度数值数组
  @ApiProperty({ title: '经纬度', required: false })
  @IsArray({ message: '经纬度必须是数组' })
  @IsNumber(
    { allowNaN: false },
    { each: true, message: '经纬度必须是数字数组' },
